# Parameters
$mediaItemPath = '/sitecore/media library/Migration/TaxonomyData'  # Path to the JSON file in the Media Library
...
foreach ($record in $data) {
    ...
    $itemId = $record.Id
    $parentPath = $record.ParentPath -replace '/sitecore/content/WTW/Taxonomy', '/sitecore/content/WTW/WTW/Taxonomy'
    ...
    if ($null -ne $item) {
        ...
        try {
            foreach ($field in $record.PSObject.Properties) {
                $fieldName = $field.Name  # No prefix removal needed
                $fieldValue = $field.Value

                # Only update fields that are not standard metadata columns
                if ($fieldName -notin @('Id', 'Name', 'ParentPath', 'Language')) {
